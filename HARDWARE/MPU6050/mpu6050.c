...
//IIC读Z轴陀螺仪加速度
//返回值：Z轴加速度
float MPU_Read_Z_Accel(void)
{
	float Accel_Z;
	Accel_Z = (MPU_Read_Byte(MPU_ACCEL_ZOUTH_REG) << 8) + MPU_Read_Byte(MPU_ACCEL_ZOUTL_REG);
...
float MPU_Data_Get_Filter(void)
{
	float Accel_Y,Accel_Z,Gyro_X,Gyro_Z;
	Gyro_X = MPU_Read_X_Gyro();
	Gyro_Z = MPU_Read_Z_Gyro();
	Accel_Y = MPU_Read_Y_Accel();
	Accel_Z = MPU_Read_Z_Accel();
	Accel_Angle = atan2(Accel_Y,Accel_Z)*180/PI;             //计算倾角
	Gyro_X = Gyro_X/16.4;                                    //陀螺仪量程转换	
	gyroz = Gyro_Z;
	return Kalman_Filter(Accel_Angle,Gyro_X);
}
