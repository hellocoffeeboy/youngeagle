...
void send_cur_set_angel(float Pitch_Set_Pos,float Pitch_Cur_Pos,float Roll_Set_Pos,float Roll_Cur_Pos)
{
    ...
    p=(unsigned char *)&Pitch_Set_Pos;
    tbuf[0]=(unsigned char)(*(p+3));
    tbuf[1]=(unsigned char)(*(p+2));
    tbuf[2]=(unsigned char)(*(p+1));
    tbuf[3]=(unsigned char)(*(p+0));
     
    p=(unsigned char *)&Pitch_Cur_Pos;
    tbuf[4]=(unsigned char)(*(p+3));
    tbuf[5]=(unsigned char)(*(p+2));
    tbuf[6]=(unsigned char)(*(p+1));
    tbuf[7]=(unsigned char)(*(p+0));
	
    p=(unsigned char *)&Roll_Set_Pos;
    tbuf[8]=(unsigned char)(*(p+3));
    tbuf[9]=(unsigned char)(*(p+2));
    tbuf[10]=(unsigned char)(*(p+1));
    tbuf[11]=(unsigned char)(*(p+0));
     
    p=(unsigned char *)&Roll_Cur_Pos;
    tbuf[12]=(unsigned char)(*(p+3));
    tbuf[13]=(unsigned char)(*(p+2));
    tbuf[14]=(unsigned char)(*(p+1));
    tbuf[15]=(unsigned char)(*(p+0));
     
    usart1_niming_report(0XA1,tbuf,16);//自定义帧,0XA1
}
//发送pitch和roll方向PWM
void send_pwm(u16 pitch_pwm,u16 roll_pwm)
{
	u8 tbuf[4]; 
	tbuf[0]=(pitch_pwm>>8)&0XFF;
	tbuf[1]=pitch_pwm&0XFF;
	tbuf[2]=(roll_pwm>>8)&0XFF;
	tbuf[3]=roll_pwm&0XFF;
	usart1_niming_report(0XA2,tbuf,4);//自定义帧,0XA2	
}
